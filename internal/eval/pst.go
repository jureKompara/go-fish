...
/*
PeSTO tapered eval (MG/EG + phase blend)

Your piece indexing:
  KNIGHT=0, BISHOP=1, ROOK=2, QUEEN=3, PAWN=4, KING=5

Assumptions:
- p.PieceBB[color][piece] uses the same piece indices above.
- Square indexing is a1=0..h1=7, a8=56..h8=63 (so vertical flip is sq^56).
- PST tables below are stored in BLACK POV (your convention), so WHITE uses flipped square.
- Returns eval from side-to-move POV.
*/
...
var mgValuePesto = [6]int32{82, 337, 365, 477, 1025, 0}
var egValuePesto = [6]int32{94, 281, 297, 512, 936, 0}
...
// Map your engine piece index -> PeSTO piece type index (pawn..king)
func pestoIdxOfEnginePiece(piece int) int {
	switch piece {
	case engine.PAWN: // 4
		return 0
	case engine.KNIGHT: // 0
		return 1
	case engine.BISHOP: // 1
		return 2
	case engine.ROOK: // 2
		return 3
	case engine.QUEEN: // 3
...
func init() {
	for piece := range 6 {
		pi := pestoIdxOfEnginePiece(piece)

		mgMat := mgValuePesto[pi]
		egMat := egValuePesto[pi]

		for sq := range 64 {
			PSQMG[engine.WHITE][piece][sq] = mgMat + mgPSTBlack[pi][sq]
			PSQEG[engine.WHITE][piece][sq] = egMat + egPSTBlack[pi][sq]

			PSQMG[engine.BLACK][piece][sq] = mgMat + mgPSTBlack[pi][sq^56]
			PSQEG[engine.BLACK][piece][sq] = egMat + egPSTBlack[pi][sq^56]
		}
	}
}

// EvalPeSTO returns tapered PeSTO eval from side-to-move POV.
func EvalPeSTO(p *engine.Position) int32 {
	var mgW, mgB int32
	var egW, egB int32
	var phase int32
	...
	for piece := range 6 {
		...
		bb := white[piece]
		if bb != 0 {
			phase += int32(bits.OnesCount64(bb)) * phaseIncPesto[pi]
			for bb != 0 {
				sq := engine.PopLSB(&bb)
